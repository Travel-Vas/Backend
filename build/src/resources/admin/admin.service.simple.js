...
// Simplified service implementations to avoid missing model errors
const DashboardService = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const photographersPromise = user_model_1.default.countDocuments({
            role: constants_1.UserRole.PHOTOGRAPHER
        });
        const revenuePromise = total_revenue_1.calculateTotalRevenueWithAggregation;
        const pendingPayments = transaction_model_1.TransactionModel.find({
            txnStatus: transaction_interface_1.TxnStatus.PENDING
        }).lean().exec();
        const completedPayments = transaction_model_1.TransactionModel.find({
            txnStatus: transaction_interface_1.TxnStatus.COMPLETED
        }).lean().exec();
        const failedPayments = transaction_model_1.TransactionModel.find({
            txnStatus: transaction_interface_1.TxnStatus.FAILED
        }).lean().exec();
        const today = new Date();
        const firstDayOfMonth = new Date(today.getFullYear(), today.getMonth(), 1);
        const newCustomers = user_model_2.default.find({
            created_at: { $gte: firstDayOfMonth, $lte: today }
        }, "business_name role status").lean().exec();
        const [photographers, revenue, newCustomer, pendingPayment, completedPayment, failedPayment] = yield Promise.all([
...
const PhotographersProfileService = (user_id) => __awaiter(void 0, void 0, void 0, function* () {
    const profile = yield user_model_2.default.findOne({ _id: user_id })
        .select("-password")
